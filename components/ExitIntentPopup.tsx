  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="w-12 h-1 bg-emerald-500 rounded-full mb-5" />

        <h2 className="text-2xl font-extrabold text-white mb-3 leading-tight">
          Not sure which cover your organisation needs?
        </h2>
        <p className="text-gray-300 text-sm leading-relaxed mb-6">
          Our specialists match charities, trusts, and community organisations with the right cover — public liability, professional indemnity, and more. Registered Financial Service Providers, no obligation.
        </p>
        ...
        <div className="flex flex-col gap-3">
          <Link
            href="/contact/"
            onClick={onClose}
            className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 px-6 rounded-xl text-center transition-colors text-sm"
          >
            Get a Quote →
          </Link>
          <button
            onClick={onClose}
            className="w-full text-gray-400 hover:text-gray-300 text-xs py-2 transition-colors"
          >
...
function MobileStickyBar({ onClose }: { onClose: () => void }) {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-slate-900 border-t-2 border-emerald-600 shadow-2xl">
      <div className="flex items-center gap-3 px-4 py-3">
        <div className="flex-1 min-w-0">
          <p className="text-white font-semibold text-sm leading-tight">Ready to protect your organisation?</p>
          <p className="text-gray-400 text-xs mt-0.5">No obligation · Registered advisers</p>
        </div>
        <Link
          href="/contact/"
          className="shrink-0 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 px-4 rounded-xl text-sm transition-colors"
        >
          Get a Quote →
        </Link>
        <button
